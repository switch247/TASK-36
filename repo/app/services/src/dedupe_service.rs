...
impl DedupeService {
    pub fn guided_merge_match(left: DedupeCandidate<'_>, right: DedupeCandidate<'_>) -> bool {
        is_guided_merge_match(
            left.id, right.id, left.name, right.name, left.dob, right.dob,
        )
    }
    ...
    pub fn should_surface_guided_merge(
        left: DedupeCandidate<'_>,
        right: DedupeCandidate<'_>,
    ) -> Result<bool> {
        let score = Self::guided_merge_similarity(left.name, right.name);
        Ok(
            (score >= 0.90 && left.dob == right.dob)
