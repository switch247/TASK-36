...
mod tests {
    ...
    async fn reporting_service_returns_structured_report_rows() {
        ...
        common::factory_asset(&app.pool, "asset-report-unit", "BOOK-REPORT-UNIT", "sess-report-unit", &coord_id).await;
        ...
        assert!(incidents.iter().any(|row| row.session_id == "sess-report-unit"));

        let returns = service.return_rates().await.expect("returns");
        assert!(returns.iter().any(|row| row.session_id == "sess-report-unit"));

        let inventory = service.materials_inventory().await.expect("inventory");
        assert!(inventory.iter().any(|row| row.asset_id == "asset-report-unit"));
    }
    ...
    async fn reporting_service_operations_alerts_surface_all_alert_types() {
        ...
        common::factory_asset(&app.pool, "asset-alert-hi", "BOOK-ALERT-HI", "sess-alert-hi", &coord_id).await;
        ...
        common::factory_session(&app.pool, "sess-alert-low-return", "Template A", 60, &coord_id).await;
        common::factory_asset(&app.pool, "asset-alert-low-return", "BOOK-ALERT-LOW", "sess-alert-low-return", &coord_id).await;
