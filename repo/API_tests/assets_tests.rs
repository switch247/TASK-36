...
async fn create_asset_by_coordinator_persists() {
    ...
    let resp = attach_auth(
        app.client.post("/api/v1/assets").json(&asset_payload(
            "asset-create-1",
            "BOOK-CREATE-1",
            "sess-asset-1",
        )),
...
async fn create_asset_duplicate_booklet_code_returns_409() {
    ...
    assert_eq!(
        second.status(),
        Status::Conflict,
        "unique booklet_code must trigger 409"
    );
...
async fn update_asset_not_found_returns_404() {
    ...
    let resp = attach_auth(
        app.client
            .put("/api/v1/assets/missing-asset")
            .json(&payload),
        &headers,
    )
    .dispatch()
...
async fn delete_asset_forbidden_for_proctor() {
    ...
    let resp = attach_auth(
        app.client.delete("/api/v1/assets/asset-proc"),
        &proctor_headers,
    )
    .dispatch()
    .await;
